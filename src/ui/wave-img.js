...
class WaveGallery extends HTMLElement {
    ...
    constructor() {
        ...
        .dots i:hover,.action wave-icon:hover{
        ...
        .btn-close:hover wave-icon,.btn-close:focus wave-icon{
            color:var(--errorColor,#f4615c);
        }
        </style>
        <button id="close" class="btn-close"><wave-icon name="close"></wave-icon></button>
        <slot id="slot"></slot>
        <a class="action" id="dots"><wave-icon name="caret-left" class="left"></wave-icon><div class="dots"></div><wave-icon name="caret-right" class="right"></wave-icon></a>
        `
    }
    ...
    attributeChangedCallback(name, oldValue, newValue) {
        if (name == 'open' && this.shadowRoot) {
            if (newValue == null) {
                document.querySelector(`wave-img[index="${this.indexlist[this.index]}"]`).focus();
            }
        }
    }
...
if (!customElements.get('wave-gallery')) {
    customElements.define('wave-gallery', WaveGallery);
}

export default class WaveImg extends HTMLElement {
    ...
    constructor() {
        ...
        <div class="placeholder" id="placeholder" style="${this.ratio ? 'padding-top:' + this.ratio : ''}"><div class="placeholder-icon"><wave-icon path="M928 160H96c-17.7 0-32 14.3-32 32v640c0 17.7 14.3 32 32 32h832c17.7 0 32-14.3 32-32V192c0-17.7-14.3-32-32-32z m-40 632H136v-39.9l138.5-164.3 150.1 178L658.1 489 888 761.6V792z m0-129.8L664.2 396.8c-3.2-3.8-9-3.8-12.2 0L424.6 666.4l-144-170.7c-3.2-3.8-9-3.8-12.2 0L136 652.7V232h752v430.2z"></wave-icon>${this.alt}</div></div>
        <wave-icon class="view" name='View'></wave-icon>
        <img>
        <div class="loading">
            <div class="animation">
    ...
    load(src, hasload) {
        ...
        img.onerror = () => {
            ...
            //window['WaveGallery' + this.gallery] && window['WaveGallery' + this.gallery].remove(this.WaveImgIndex);
        }
    }

    initgallery() {
        if (this.gallery !== null) {
            if (!window['WaveGallery' + this.gallery]) {
                window['WaveGallery' + this.gallery] = new WaveGallery();
                document.body.appendChild(window['WaveGallery' + this.gallery]);
            }
            this.img.setAttribute('tabindex', 0);
            this.setAttribute('index', this.WaveImgIndex);
            this.img.addEventListener('click', () => {
                if (!this.default) {
                    window['WaveGallery' + this.gallery].show(this.WaveImgIndex);
                }
            })
            ...
            img.style.order = this.WaveImgIndex;
            img.dataset.index = this.WaveImgIndex;
            window['WaveGallery' + this.gallery].add(img, this.WaveImgIndex);
        }
    }

    connectedCallback() {
        if (window.WaveImgIndex > -1) {
            window.WaveImgIndex++;
        } else {
            window.WaveImgIndex = 0;
        }
        this.WaveImgIndex = window.WaveImgIndex;
...
if (!customElements.get('wave-img')) {
    customElements.define('wave-img', WaveImg);
}
