...
export default class WavePagination extends HTMLElement {
    ...
    constructor() {
        ...
        wave-button[tabindex]{
            justify-content: center;
            align-items: center;
            pointer-events: none;
        }
        .page-ellipsis wave-icon{
            margin:auto;
        }
        wave-button[current] {
        ...
        <wave-button type="flat" id="right">
    ...
    render(pagesize,total){
        ...
        if(this.simple){
            const html = `<wave-button class="simple-page" tabindex="-1" type="flat">${current} / ${this.count}</wave-button>`;
            this.page.innerHTML = html;
        }else{
            const html = Array.from({length:this.count},(el,i)=>i).splice(0,9).map(el=>`<wave-button ${el+1==current?"current":""} type="flat" data-current="${el+1}">${el+1}</wave-button>`).join('');
            this.page.innerHTML = html;
        }
        this.updatePage(current);
    }
    ...
    updatePage(current=this.current){
        ...
        if(this.simple){
            ...
            if( this.count>9 ){
                ...
                this.page.querySelectorAll('wave-button').forEach((el,i)=>{
    ...
    connectedCallback() {
        ...
        this.page.addEventListener('click',(ev)=>{
            const item = ev.target.closest('wave-button');
            if(item){
...
if(!customElements.get('wave-pagination')){
    customElements.define('wave-pagination', WavePagination);
}
