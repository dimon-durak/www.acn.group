...
class WaveTabContent extends HTMLElement {
...
if(!customElements.get('wave-tab-content')){
    customElements.define('wave-tab-content', WaveTabContent);
}

export default class WaveTab extends HTMLElement {
    ...
    connectedCallback() {
        ...
        this.slots.addEventListener('slotchange', ()=>{
            ...
            slots.forEach((item,index)=>{
                if( item.tagName === 'wave-TAB-CONTENT' ){
                    if(item.key===null){
                        item.key = index;
                    }
                    html += `<wave-button class="nav-item ${item.key===this.activekey?'active':''}" icon=${item.icon} ${item.disabled!==null?"disabled":""} data-key=${item.key}>${item.label}</wave-button>`;
                }
            })
    ...
    attributeChangedCallback (name, oldValue, newValue) {
        if( name == 'activekey' && this.tab){
            ...
            ::slotted(wave-tab-content:not([key="${this.activekey}"])){
                height:0;
                overflow:visible;
            }
            `
...
if(!customElements.get('wave-tab')){
    customElements.define('wave-tab', WaveTab);
}
