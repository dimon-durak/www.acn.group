...
export default class WaveInput extends HTMLElement {
    ...
    constructor({multi}={}) {
        ...
        .btn-number wave-button:hover{
            flex:1.5;
        }

        wave-button:not([disabled]):hover,wave-button:not([disabled]):focus-within{
        ...
        <wave-tips id="input-con" dir="${this.errordir}" type="error">
            ${
                this.icon?
                '<wave-icon class="icon-pre" name='+this.icon+'></wave-icon>'
                :
                ''
            }
            ...
            ${
                ...
                '<wave-button id="btn-pass" class="btn-right" icon="eye-close" type="flat" shape="circle"></wave-button>'
                :
                ''
            }
            ${
                this.type === 'search'&&!multi?
                '<wave-button id="btn-search" class="btn-right" icon="search" type="flat" shape="circle"></wave-button>'
                :
                ''
            }
            ${
                this.type === 'number'&&!multi?
                '<div class="btn-right btn-number"><wave-button id="btn-add" icon="up" type="flat"></wave-button><wave-button id="btn-sub" icon="down" type="flat"></wave-button></div>'
                :
                ''
            }
        </wave-tips>
...
class WaveTextarea extends WaveInput {
    constructor() {
        super({multi:true});
    }
}

if(!customElements.get('wave-input')){
    customElements.define('wave-input', WaveInput);
}
if(!customElements.get('wave-textarea')){
    customElements.define('wave-textarea', WaveTextarea);
}
