...
import './wave-input.js';

class WaveDialog extends HTMLElement {
    ...
    constructor({type}={}) {
        ...
        <div class="dialog">
            <wave-icon id="dialog-type" class="dialog-type"></wave-icon>
            <div class="dialog-content">
                <div class="dialog-title" id="title">${this.title}</div>
                <wave-button class="btn-close" id="btn-close" icon="close"></wave-button>
                <div class="dialog-body">
                    <slot></slot>
                    ${(type||this.type)==="prompt"?"<wave-input></wave-input>":""}
                </div>
                <div class="dialog-footer">
                    <wave-button id="btn-cancel">${this.canceltext}</wave-button>
                    <wave-button id="btn-submit" type="primary">${this.oktext}</wave-button>
                </div>
            </div>
        </div>
        `
    ...
    connectedCallback() {
        ...
        this.input = this.shadowRoot.querySelector('wave-input');
        this.clientWidth;
...
if(!customElements.get('wave-dialog')){
    customElements.define('wave-dialog', WaveDialog);
}

export default {

    alert: function() {
        const dialog = new WaveDialog();
    ...
    confirm: function() {
        //const dialog = document.createElement('wave-dialog');
        const dialog = new WaveDialog();
        document.body.appendChild(dialog);
    ...
    prompt: function() {
        const dialog = new WaveDialog({type:'prompt'});
        document.body.appendChild(dialog);
        dialog.btnCancel.style.visibility = 'visible';
        dialog.remove = true;
