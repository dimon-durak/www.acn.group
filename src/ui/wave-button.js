...
export default class WaveButton extends HTMLElement {
    ...
    constructor() {
        ...
        <${this.href?'a':'button'} ${this.htmltype?'type="'+this.htmltype+'"':''} ${(this.download&&this.href)?'download="'+this.download+'"':''} ${this.href?'href="'+this.href+'" target="'+this.target+'" rel="'+this.rel+'"':''} class="btn" id="btn"></${this.href?'a':'button'}>${!this.loading && this.icon && this.icon!='null'?'<wave-icon id="icon" name='+this.icon+'></wave-icon>':''}<slot></slot>
        `
    }
    ...
    connectedCallback() {
        ...
        this.load = document.createElement('wave-loading');
...
if(!customElements.get('wave-button')){
    customElements.define('wave-button', WaveButton);
}

class WaveButtonGroup extends HTMLElement {
    ...
    constructor() {
        ...
        ::slotted(wave-button:not(:first-of-type):not(:last-of-type)){
            border-radius:0;
        }
        ::slotted(wave-button){
            margin:0!important;
        }
        ::slotted(wave-button:not(:first-of-type)){
            margin-left:-1px!important;
        }
        ::slotted(wave-button[type]:not([type="dashed"]):not(:first-of-type)){
            margin-left:1px!important;
        }
        ::slotted(wave-button:first-of-type){
            border-top-right-radius: 0;
            border-bottom-right-radius: 0px;
        }
        ::slotted(wave-button:last-of-type){
...
if(!customElements.get('wave-button-group')){
    customElements.define('wave-button-group', WaveButtonGroup);
}
