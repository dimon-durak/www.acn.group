...
import './wave-popover.js';

class WaveOptionGroup extends HTMLElement {
    constructor() {
        super();
...
if(!customElements.get('wave-optgroup')){
    customElements.define('wave-optgroup', WaveOptionGroup);
}

class WaveOption extends HTMLElement {
    ...
    constructor() {
        ...
        <wave-button id="option" class="option" type="flat" ${this.disabled?"disabled":""}><slot></slot></wave-button>
        `
    }
...
if(!customElements.get('wave-option')){
    customElements.define('wave-option', WaveOption);
}

export default class WaveSelect extends HTMLElement {
    ...
    constructor() {
        ...
        :host(:focus-within) wave-popover,:host(:active) wave-popover{ 
            z-index: 2;
        }
        wave-tips{
            display:block;
        ...
        :host([search]) wave-popcon::before{
            display:none;
        ...
        :host([empty]) wave-popcon::before{
            display:block;
        }
        </style>
        <style id="filter"></style>
        <wave-popover id="root" ${this.search?"accomplish":""}>
            <wave-tips id="tip" type="error">
                <${this.search?'wave-input':'wave-button'} id="select" debounce="200" readonly ${this.disabled? "disabled" : ""} ${this.type?("type="+this.type):""}>${this.search?"":'<span id="value"></span>'}<svg class="arrow" viewBox="0 0 1024 1024"><path d="M884 256h-75c-5.1 0-9.9 2.5-12.9 6.6L512 654.2 227.9 262.6c-3-4.1-7.8-6.6-12.9-6.6h-75c-6.5 0-10.3 7.4-6.5 12.7l352.6 486.1c12.8 17.6 39 17.6 51.7 0l352.6-486.1c3.9-5.3 0.1-12.7-6.4-12.7z"></path></svg></${this.search?'wave-input':'wave-button'}>
            </wave-tips>
            <wave-popcon id="options">
                <slot id="slot"></slot>
            </wave-popcon>
        </wave-popover>
        `
    ...
    connectedCallback() {
        this.form = this.closest('wave-form');
        ...
        this.options.addEventListener('click',(ev)=>{
            this.focus();
            const item = ev.target.closest('wave-option');
            if(item){
        ...
        this.options.addEventListener('close',(ev)=>{
            if(this.search){
                ...
                this.nodes = [...this.querySelectorAll(`wave-option:not([disabled])`)];
                this.filter.textContent = '';
                this.empty = false;

            }
            const place = this.querySelector(`wave-option[focusin]`);
            const current = this.querySelector(`wave-option[selected]`);
            if(place){
                place.focusin = false;
            }
            if(current){
        ...
        if(this.search){
            ...
            this.select.addEventListener('input',(ev)=>{
                ...
                if(value===""){
                    ...
                    this.nodes = [...this.querySelectorAll(`wave-option[key*="${value}" i]:not([disabled])`)];
                    this.filter.textContent = `
                    :host([search]) ::slotted(wave-option:not([key*="${value}" i]))
                    {
                        display:none;
                    }
                    `
    ...
    set value(value) {
        ...
        if (value !== this.value) {
            this.$value = value;
            const pre = this.querySelector(`wave-option[selected]`);
            if(pre){
                pre.selected = false;
                pre.focusin = false;
            }
            const cur = this.querySelector(`wave-option[value="${value}"]`)||this.querySelector(`wave-option`);
...
if (!customElements.get('wave-select')) {
    customElements.define('wave-select', WaveSelect);
}
