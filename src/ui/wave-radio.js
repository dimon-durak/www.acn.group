...
export default class WaveRadio extends HTMLElement {
    ...
    tocheck() {
        const selector = this.group?`wave-radio[checked]`:`wave-radio[name="${this.name}"][checked]`;
    ...
    connectedCallback() {
        this.group = this.closest('wave-radio-group');
...
if(!customElements.get('wave-radio')){
    customElements.define('wave-radio', WaveRadio);
}

class WaveRadioGroup extends HTMLElement {
    ...
    constructor() {
        ...
        :host([disabled]) ::slotted(wave-radio){
            pointer-events: none;
            opacity:.6;
        }
        wave-tips[show=show]{
        ...
        <wave-tips id="tip" type="error"><slot></slot></wave-tips>
        `
    }
    ...
    get value() {
        const radio = this.querySelector('wave-radio[checked]');
    ...
    connectedCallback() {
        ...
        this.slots.addEventListener('slotchange',()=>{
            this.elements  = this.querySelectorAll('wave-radio');
            this.value = this.defaultvalue;
            this.elements.forEach(el=>{
...
if(!customElements.get('wave-radio-group')){
    customElements.define('wave-radio-group', WaveRadioGroup);
}
